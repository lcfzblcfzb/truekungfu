...
func _input(event):
	...
	if(event is InputEventMouse):
		...
		if event.is_action_pressed("attack"):
			...
			var pool = Tool.PoolDict.get(NewActionEvent) as ObjPool
			var newActionEvent = pool.instance([lastByte,moving_position_array,attack_begin_time,OS.get_ticks_msec()])
	...
	if event.is_action("ui_right")||event.is_action("ui_left")||event.is_action("ui_up")||event.is_action("ui_down"):
		...
		var pool = Tool.PoolDict.get(MoveEvent) as ObjPool
		var moveEvent = pool.instance([input_vector,event.is_echo()])

		emit_signal("NewFightMotion",moveEvent)
